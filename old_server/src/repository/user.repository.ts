...
export class UserRepository extends AbstractRepository<User> {
  ...
  public async findById(id: number): Promise<User | null> {
    const query = `SELECT ${TableName.User}.id, ${TableName.User}.phone, ${TableName.User}.isConnected,
  ...
  public async findByPhone(phone: string): Promise<User | null> {
    const query = `SELECT ${TableName.User}.id, ${TableName.User}.phone, ${TableName.User}.isConnected,
      ...
      ${TableName.Bill}.userId, ${TableName.Bill}.balance, ${TableName.Bill}.currency FROM 
      ${TableName.User} LEFT JOIN ${TableName.Bill} ON
      ${TableName.User}.billId = ${TableName.Bill}.id WHERE ${TableName.User}.phone = ?; `;
    const values = [phone];
    return this.connector.query(query, values).then(rows => rows[0] ? this.extractBill(rows[0]) : null);
  }
  ...
  public async create(user: User): Promise<void> {
    ...
    query = `INSERT INTO ${TableName.User} (phone, password, isConnected, country, role)
      VALUES (?, ?, ?, ?, ?); `;
    values = [user.phone, user.password, user.isConnected, user.country, user.role];
    ...
    query = `INSERT INTO ${TableName.Bill} (userId, balance, currency)
      ...
      ORDER BY id DESC LIMIT 1) WHERE ${TableName.User}.phone = ?; `;
    values = [user.bill.balance, user.bill.currency, user.phone];
  ...
  public async findAllAbonents(): Promise<User[]> {
    ...
    query = `SELECT ${TableName.User}.id, ${TableName.User}.phone, ${TableName.User}.isConnected,
