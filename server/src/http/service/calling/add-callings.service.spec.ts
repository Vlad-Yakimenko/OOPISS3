...
import { BadRequestException, ForbiddenException, NotFoundException } from "@app/http/error";
...
describe('`AddCallingsService`', () => {
  ...
  it('should be instance of `AddCallingsService`', () => {
    ...
    expect(new AddCallingsService()).toBeInstanceOf(AddCallingsService);
  });
  ...
  describe('`addCallings`', () => {
    ...
    it('should process all provided callings and return OK status if user has enough money', async () => {
      ...
      callings.forEach((calling) => {
        expect(mockCallingRepository.create).toHaveBeenCalledWith(calling);
      });
